...
import { createConnection, sendEmails } from '@notifications/queues';
...
async function consumeAuthEmailMessage(channel: Channel): Promise<void> {
  try {
    ...
    channel.consume(jobberQueue.queue, async (msg: ConsumeMessage | null) => {
      ...
      await sendEmails(template, receiverEmail, locals);
      channel.ack(msg!);
    });
  } catch (error) {
  ...
  try {
    ...
    channel.consume(jobberQueue.queue, async (msg: ConsumeMessage | null) => {
      ...
      if (template === 'orderPlaced') {
        await sendEmails('orderPlaced', receiverEmail, locals);
        await sendEmails('orderReceipt', receiverEmail, locals);
      } else {
        await sendEmails(template, receiverEmail, locals);
      }
